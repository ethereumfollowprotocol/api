...
export function allFollowing(users: Hono<{ Bindings: Environment }>, services: Services) {
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: <explanation>
  users.get('/:addressOrENS/allFollowing', includeValidator, async context => {
    ...
    const cacheTarget = `users/${addressOrENS}/allFollowing?limit=${limit}&offset=${offset}&sort=${direction}&tags=${tagsToSearch.join(',')}`
    if (cache !== 'fresh') {
    ...
    const followingListRecords: FollowingResponse[] = await efp.getAllUserFollowingByAddressTagSort(
      address,
      limit,
      offset,
      tagsToSearch,
